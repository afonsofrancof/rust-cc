...
use my_dns::dns_structs::dns_message::{QueryType, DNSMessage, DNSQueryInfo, DNSMessageData, DNSEntry, DNSMessageHeaders};
...
fn test_client_server() {
    ...
    match client.join() {
        Ok(msg) => {
            let dns_query_info = DNSQueryInfo{ name: "www.example.com".to_string() ,type_of_value: vec![QueryType::A] } ;
            //Fill in response_values
            let mut response_values = HashMap::new();
            response_values.insert(QueryType::A, vec![DNSEntry{ name: "www.example.com.".to_string(), type_of_value: "A".to_string(), value: "10.3.3.1".to_string(), ttl: 86400, priority: Some(200)}]);
            //Fill in authorities_values
            let authorities_values = vec![DNSEntry{ name: "example.com.".to_string(), type_of_value: "NS".to_string(), value: "ns1.example.com.".to_string(), ttl:86400, priority: None }];
            //Fill in extra_values
            let extra_values = vec![DNSEntry{ name: "ns1.example.com.".to_string(), type_of_value: "A".to_string(), value: "10.2.2.2".to_string(), ttl: 86400, priority: None}];
